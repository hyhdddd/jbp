...
<tr bgcolor="#FFFFFF"  align='center'> 
	<td valign='top' align='center'>
		...
		<tr height="30"> 
			...
			<a href="/result/a_result_list_2_excel.asp?g_article_idx1=<%=g_article_idx1%>&start_date=<%=start_date%>&end_date=<%=end_date%>"><img src="/images/button/btn_excel.gif" border=0 align="absmiddle"></a>
			</td>
		</tr>
		<tr>
			<td align='right'>
				<form name="frm" method="post" action="a_result_list_2.asp">
		...
		<tr height=30>                     
			<td width="150" class="head"> 날 짜 </font></td>
			<% For i=1 To article_cnt %>
			<td width="120" class="head"> <%=article_name(i)%> </td>
			<% Next %>
...
%>		  
		<tr height=30>
			<td class="con5">
				<a href="a_result_list_2_1.asp?start_date=<%=date_add%>&end_date=<%=date_add%>&g_article_idx1=<%=g_article_idx1%>">
				<%=date_add%></a>
			</td>
			<% For j=1 To article_cnt %>
			<td class="con4">
				<% If article_cnt1(j)>0 Then %>
				<a href="a_result_list_2_1.asp?start_date=<%=date_add%>&end_date=<%=date_add%>&start_day=<%=db_day%>&g_article_idx1=<%=g_article_idx1%>&g_article_idx2=<%=article_idx(j)%>"><%=article_cnt1(j)%></a>
				<% Else %>
					&nbsp;
				<% End if %>
			</td>
			...
			<Td class="con4"> 
				...
				<a href="a_result_list_2_1.asp?start_date=<%=date_add%>&end_date=<%=date_add%>&g_article_idx1=<%=g_article_idx1%>"><%=article_sum%>
				<% Else %>
					&nbsp;
				<% End If %>
			</td>
...
%>	
		<tr height=30>
			...
			<td class="con4">
				...
				<a href="a_result_list_2_1.asp?start_date=<%=start_date%>&end_date=<%=end_date%>&g_article_idx1=<%=g_article_idx1%>&g_article_idx2=<%=article_idx(j)%>"><%=article_cnt_total(j)%></td>
				<%
				   Else 
					response.write "&nbsp;"
				   End if 
			...
			<Td class="con4"> 
				<% If article_sum_total>0 Then %>
					<a href="a_result_list_2_1.asp?start_date=<%=start_date%>&end_date=<%=end_date%>&g_article_idx1=<%=g_article_idx1%>"><%=article_sum_total%></a>
				<% Else %>
					&nbsp;
				<% End If %>
			</td>
