...
<tr bgcolor="#FFFFFF"  align='center'> 
	<td valign='top' align='center'>		
		...
		<tr height="30"> 
			<td colspan="2">검색 내용 : 나의 일자별 개통현황</td>
		</tr>				
		<tr height="30"> 
			<td colspan="2">검색 상품 : <%=article_com%></td>
		</tr>
		<tr height="30"> 
			<td colspan="2">검색 기간 : (<%=start_date%> ~ <%=end_date%>)</td>
		</tr>
		<tr>
			<td align='right'>
				<form name="frm" method="post" action="p_result_list_2.asp">
...
%>		  
		<tr height=30>
			<td class="con5">
				<a href="p_result_list_2_1.asp?start_year=<%=db_year%>&start_month=<%=db_month%>&start_day=<%=db_day%>&g_article_idx1=<%=g_article_idx1%>">
				<%=date_add%></a>
			</td>
			<% For j=1 To article_cnt %>
			<td class="con4">
				<% If article_cnt1(j)>0 Then %>
				<a href="p_result_list_2_1.asp?start_year=<%=db_year%>&start_month=<%=db_month%>&start_day=<%=db_day%>&g_article_idx1=<%=g_article_idx1%>&g_article_idx2=<%=article_idx(j)%>"><%=article_cnt1(j)%></a>
				<% Else %>
					&nbsp;
				<% End if %>
			</td>
			...
			<Td class="con4"> 
				...
				<a href="p_result_list_2_1.asp?start_year=<%=db_year%>&start_month=<%=db_month%>&start_day=<%=db_day%>&g_article_idx1=<%=g_article_idx1%>"><%=article_sum%>
				<% Else %>
					&nbsp;
				<% End If %>
			</td>
...
%>	
		<tr height=30>
			<td class="con5">합  계</td>
			<% For j=1 To article_cnt %>
			<td class="con4">
				<% If article_cnt_total(j)>0 Then %>
				<a href="p_result_list_2_2.asp?start_date=<%=start_date%>&end_date=<%=end_date%>&g_article_idx1=<%=g_article_idx1%>&g_article_idx2=<%=article_idx(j)%>"><%=article_cnt_total(j)%>
				<% 
				   Else 
					response.write "&nbsp;"
				   End if 
			...
			<Td class="con4"> 
				<% If article_sum_total>0 Then %>
					<a href="p_result_list_2_2.asp?start_date=<%=start_date%>&end_date=<%=end_date%>&g_article_idx1=<%=g_article_idx1%>&g_article_idx2=<%=article_idx(j)%>"><%=article_sum_total%></a>
				<% Else %>
					&nbsp;
				<% End If %>
			</td>
