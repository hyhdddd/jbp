  ...
  strsql = " select count(*) from code_article where "
  strSql = strSql & " article_step1="& g_article_idx1 &" and article_step2>0"
  Set objRs=objCnn.Execute(strSql)
...
<tr bgcolor="#FFFFFF"  align='center'> 
	<td valign='top' align='center'>
		...
		<tr>
			<td align='right'>
				<form name="frm" method="post" action="y_result_list_4.asp">
...
<%
  strSql = " select m_id , m_name from emp_list where "
  strSql = strSql & " m_dealer_code in (select m_dealer_code from emp_list "
...
%>		  
		<tr height=30>
			<td class='con5'>
				<a href="y_result_list_4_1.asp?start_date=<%=start_date%>&end_date=<%=end_date%>&g_article_idx1=<%=g_article_idx1%>&m_id=<%=m_id%>">
				<%=m_name%></a>
			</td>
			<% For j=1 To article_cnt %>
			<td class='con4'>
				<% If article_cnt1(j)>0 Then %>
				<a href="y_result_list_4_1.asp?start_date=<%=start_date%>&end_date=<%=end_date%>&g_article_idx1=<%=g_article_idx1%>&m_id=<%=m_id%>&g_article_idx2=<%=article_idx(j)%>"><%=article_cnt1(j)%></a>
				<% Else %>
					&nbsp;
				<% End if %>
			</td>
			...
			<Td class='con4'> 
				...
				<a href="y_result_list_4_1.asp?start_date=<%=start_date%>&end_date=<%=end_date%>&g_article_idx1=<%=g_article_idx1%>&m_id=<%=m_id%>"><%=article_sum%>
				<% Else %>
					&nbsp;
				<% End If %>
			</td>
		...
		<tr height=30>
			...
			<td class="con4">
				<% If article_cnt_total(j)>0 Then  %>
					<a href="y_result_list_4_1.asp?start_date=<%=start_date%>&end_date=<%=end_date%>&g_article_idx1=<%=g_article_idx1%>&g_article_idx2=<%=article_idx(j)%>"><%=article_cnt_total(j)%>
					</a>
				<%
				   Else 
					response.write "&nbsp;"
			...
			<Td class="con4"> 
				<% If article_sum_total>0 Then %>
					<a href="y_result_list_4_1.asp?start_date=<%=start_date%>&end_date=<%=end_date%>&g_article_idx1=<%=g_article_idx1%>"> <%=article_sum_total%> </a>
				<% Else %>
					&nbsp;
				<% End If %>
			</td>
