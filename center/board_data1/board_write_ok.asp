  ...
  If board_idx<>"" Then	 
	  ...
	  set objRs=objCnn.Execute(strSql)
	  Alert_url " 글이 수정 되었습니다. ","board_list.asp?tname="&tname  
  Else 
	  strSql = "Select ISNULL(MAX(board_idx)+1,1) from "& tname 
	  set objRs=objCnn.Execute(strSql)
	  board_idx	= objRs(0)
	  
	  strSql = " INSERT INTO "& tname &"(board_idx, o_code, board_m_id, board_name, board_title, board_contents, "
	  ...
	  set objRs=objCnn.Execute(strSql)
	Alert_url " 글이 등록 되었습니다. ","board_list.asp?tname="&tname  
  End if
