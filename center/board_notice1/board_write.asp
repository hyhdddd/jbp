  ...
  if board_idx<>"" then
	strSql = "SELECT board_m_id, board_title, board_notice_yn, board_contents FROM "& tname & " where board_idx = " & board_idx
...
<tr bgcolor="#FFFFFF"  align='center'> 
	<td valign='top' align='center'>
		...
		<tr height=30> 
			<th width="150">등록자</th>
			<td width="850">
				<input type='text' class="txtbox" name='board_name' size=10 value='<%=board_name%>' readonly>
			</td>
		</tr>
		<% if ad_e_notice1="A" Then %>
		<tr height=30> 
			<th width='150'> 공지 글</th>
			<td width='850'>
				<input type='checkbox' name='board_notice_yn' value='Y' <% If board_notice_yn="Y" Then %> checked <% End If %>>
				게시판 공지글 등록
			</td>
		</tr>
		<% End If %>
