  ...
  if ad_e_notice2="N" then
	Alert_url session_m_name &" 님은 거래처 공지사항 권한이 없습니다. 관리자에게 문의하세요!","/main.asp"
  end If
...
<tr height="350"> 
	<td valign='top' align='center' height='400'>
		...
		<div class="sub_title"><img src="/images/common/sub07_t02.gif"></div>
		...
		<tr height=28>             
			...
			<td align="right" colspan='3'>
				<% if ad_e_notice2="W" or ad_e_notice2="E" or ad_e_notice2="A" then %>
					<a href='board_write.asp?tname=<%=tname%>'>
					<img src="/images/button/btn_new.gif" align='absmiddle'  border="0"></a>
				<% else %>
					<a href ='javascript:alert("<%=session_m_name%>님은 거래처 공지사항 등록권한이 없습니다. 관리자에 문의하세요!");'><img src="/images/button/btn_new.gif" border="0"></a> 
				<% end if %>   
			</td>
		</tr>
		</table>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" id="board_table2">		
		<tr height=30>             
			<td class="head" width=50>번호</td>
			<td class="head" width=650>제  목</td>
			<td class="head" width=150>등록인</td>
			<td width=50  class="head">파일</td>
			<td class="head" width=100>조회수</td>
			<td class="head" width=100>등록일</td>
		</tr>
  <% 
  strSql = " select A.board_idx, A.board_m_id, B.m_name as board_name, A.board_title, A.board_contents, A.board_notice_yn, "
  strSql = strSql & " A.board_readnum, A.board_writeday, A.board_fname1 "
  strSql = strSql & "	from "&tname&" A  left outer join emp_list B ON A.board_m_id=B.m_id "
  ...
  %>
		<tr height=30>
			<td class="con5" colspan='6'> 글이 존재하지 않습니다.</td>	
		  </tr>		
  <%
  else
	...
	recordcount	= objRs.recordcount  	
  	...
  	Do until objRs.EOF Or i>objRs.PageSize   	
  		...
  		if len(board_fname1)>0 then
			filepath1	= session_o_code&"/"&tname&"/"&left(board_fname1, InstrRev(board_fname1,"/"))  
		...
		If board_notice_yn="Y" then
			board_notice_chk	= "<img src='/images/button/btn_notice_top.gif'>"
  ...
  %>			  
		<tr height=30 <%=bg_txt%>>
			<td class="con5"><%=board_notice_chk%></td>	
