...
<!--
function change_article(form_name)
{
	var frm	= document.getElementById(form_name);
	frm.action="charge_buga.asp";
	frm.submit();
}
...
<tr height='400' valign='top'> 
	<td>
		...
		<form id="frm_board" name="frm_board" Method="POST" action="charge_buga_ok.asp">
		...
		<tr height='30'>
			...
			<td class="list_LBR_left">
				<select name="s_article_idx1" class="txtbox" style="width:120px;"onchange="s_article_change(this,'frm_board',this.selectedIndex);">
				...
				<select name="s_article_idx2" class="txtbox"  style="width:110px;" onchange="change_article('frm_board');">
				<option value="" <% if article_idx2="" then %> selected <% end if %>> 상품선택</option> 
				<% 
		...
		<tr height=30>             
			<th class="head_Gray_TLB" width="30%">부가서비스</th> 
  ...
  <% 
	...
	Do Until objRs.Eof
		...
		strSql = " select top 1 service_charge from charge_buga where policy_idx="&policy_idx&" and service_idx="& service_idx &" "
		Set objRs1=objCnn.Execute(strSQl)

		If objRs1.eof Or objRs1.bof Then
				service_charge		= ""
		Else
				service_charge		= objRs1("service_charge")
		End If
	%>
		<tr height='30'>
			<td class="list_LB_center"><%=service_name%></td>			
			<td class='list_LBR_center'><input type="text" class='txtbox' name="A<%=service_idx%>" size='8' onKeyUp="fncOnlyNum(this);" value="<%=service_charge%>"></td>
		</tr>
		<%
