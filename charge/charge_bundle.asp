...
<tr height='400' valign='top'> 
	<td>
		...
		<form name="frm_sale" Method="POST" action="charge_bundle_ok.asp">
		...
		<tr height='30'>
			...
			<td width='850' colspan='3'>
				<select name="s_article_idx1" style="width:120px;font-size:11px;" onblur="BlurColor(this);" onFocus="FocusColor(this);" onchange="s_article_change(this,'frm_sale',this.selectedIndex);">
				...
				%>	
					...
					strSql = " select * from code_article where article_step1="&s_article_idx1&" and article_view='Y' and article_step2>0 "
					strSql = strSql & " order by article_order Asc"
					Set objRs=objCnn.Execute(strSql)
		...
		<tr height=30>             
			<td class="head" width="420">상 품 명</td> 
			<td class="head" width="150">단독</td> 
			<td class="head" width="150">DPS</td> 
			<td class="head" width="150">TPS</td> 
		</tr>
  <%
  strSql = " select * from code_article_option A left outer "
  strSql = strSql &	"	Join charge_bundle B ON B.policy_idx="&policy_idx&" "
  strSql = strSql & "			and A.option_idx=B.article_idx "
  strSql = strSql & "		where A.article_idx="&s_article_idx2&" "
  strSql = strSql & "			and A.option_view='Y' order by option_order asc "
  ...
  <%
  	...
  	Do until objRs.EOF   	
		...
		bundle_dps_charge	= objRs("bundle_dps_charge")
		bundle_tps_charge	= objRs("bundle_tps_charge")
%>	
		<tr height=30>
			<td class="con1">&nbsp;&nbsp;<%=option_name%></td>
			<td class='con4'></td>
			<td class='con4'>
				<input type="text" class='txtbox' name="DPS_<%=option_idx%>" size='8' maxlength='6' onKeyUp="fncOnlyNum(this);" value="<%=bundle_dps_charge%>"></td>
			<td class='con4'>
				<input type="text" class='txtbox' name="TPS_<%=option_idx%>" size='8' maxlength='6' onKeyUp="fncOnlyNum(this);" value="<%=bundle_tps_charge%>"></td>			
		</tr>
  <%
   	objRs.MoveNext   		
   	loop
