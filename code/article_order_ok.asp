 <%
  ...
  if chk="D" then
  	strSql = " select ISNULL(min(article_order),0) from code_article where article_step2=0 and article_order>"&article_order  	
  ...
  Else
  	...
  	strSql = " select isnull(max(article_order),0) from code_article where article_step2=0 and article_order<"&article_order
  	set objRs=objCnn.Execute(strSql)
  ...
  Win_link "article_order.asp?page="&page
  %>
