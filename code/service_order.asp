...
<tr bgcolor="#FFFFFF"  height='400'> 
	<td valign='top' align='center'>
		...
		<form name="frm_order" method="post" action="service_order.asp">
  ...
  <%
  strSql = " select * from code_service where article_idx="&s_article_idx2&" order by service_order Asc"
  Set objRs=objCnn.Execute(strSql)
  ...
  <%  		 
	...
	Do until objRs.EOF   	
		...
		article_idx		= objRs("article_idx")
		service_order	= objRs("service_order")
...
%>
		<tr height=30>
			<td class="con5"><%=service_idx%></td>
			<td class="con2">&nbsp;<%=service_name%></td>
			<td class="con4"><%=service_view %></td>
			<td class="con4">
				<a href="service_order_ok.asp?s_article_idx1=<%=s_article_idx1%>&service_idx=<%=service_idx%>&article_idx=<%=article_idx%>&service_order=<%=service_order%>&chk=U&page=<%=page%>"><img src='/images/button/btn_up.gif' border=0 align='absmiddle'></a>
				&nbsp;|&nbsp;
				<a href="service_order_ok.asp?s_article_idx1=<%=s_article_idx1%>&service_idx=<%=service_idx%>&article_idx=<%=article_idx%>&service_order=<%=service_order%>&chk=D&page=<%=page%>"><img src='/images/button/btn_down.gif' border=0 align='absmiddle'></a>
			</td>
		</tr>
<%
