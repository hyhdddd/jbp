  ...
  g_receipt_type_chk	= WORD_CHANGE(UploadForm("g_receipt_type_chk"),20)						'상품번호
  ...
  elseIf consult_type="J" Then		'상부답변
	  ...
	  strSql = strSql & "	where g_goods_idx	= "&g_goods_idx&" "  
	  set objRs=objCnn.Execute(strSql)
  elseIf consult_type="K" Then		'VOC
	  strSql = " update customer_goods set "
	  strSql = strSql & "  g_consult_type	= '"&consult_type&"' "
	  strSql = strSql & ", g_consult_bigo	= '"&consult_bigo&"' "
	  strSql = strSql & "	where g_goods_idx	= "&g_goods_idx&" "  
  ...
  Alert_url " 이력글이 등록되었습니다.","a_custom_goods_regist.asp?g_goods_idx="&g_goods_idx&"&page="&page&"&nowblock="&nowblock&"&s_link_txt="&Server.URLEncode(s_link_txt)
  %>
