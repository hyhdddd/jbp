  ...
  g_goods_idx				= WORD_CHANGE(UploadForm("g_goods_idx"),20)							'상품번호

  consult_bigo				= WORD_CHANGE(UploadForm("consult_bigo"),0)							'비고
  consult_type				= WORD_CHANGE(UploadForm("consult_type"),20)			
  ...
  g_receipt_type_chk		= WORD_CHANGE(UploadForm("g_receipt_type_chk"),20)  	
  ...
  If consult_type="A" Then			'고객메모
	'고객메모가 올라왔는데, 현재 고객상태가 
	'상부답변이거나, 상부요청일 경우 /원상태도 자동으로 복구시킴
	'if g_receipt_type_chk="C" Or g_receipt_type_chk="I" Then
  ...
  Alert_url " 이력글이 등록되었습니다.","m_custom_goods_regist.asp?g_goods_idx="&g_goods_idx&"&page="&page&"&nowblock="&nowblock&"&s_link_txt="&Server.URLEncode(s_link_txt)
  %>
