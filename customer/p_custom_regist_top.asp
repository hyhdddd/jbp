...
<tr height='400' valign='top'> 
	<td>
		...
		<form id="frm_custom_regist" name="frm_custom_regist" action="p_custom_regist_top_ok.asp" method="post" ENCTYPE="MULTIPART/FORM-DATA"> 
		<input type="hidden" name="g_jupsu_m_id" value="<%=session_m_id%>">
		<input type="hidden" name="g_yuchi_m_id" value="<%=session_m_id%>">
		...
		<div class="button">
			<a href="p_custom_goods_list.asp?page=<%=page%>&nowblock=<%=nowblock%>&<%=s_link_txt%>"><span class="btn_m_white01"> 목록으로</span></a>
			<span onClick="custom_regist_all('P');" onmouseover="this.style.cursor='hand';"><span class="btn_m_white01"> 저장하기</span></span>
		</div>
		</form>
		<!------------------ 내용 시작  ------------------>
	</td>
