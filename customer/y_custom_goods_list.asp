  ...
  g_goods_idx_chk	= WORD_CHANGE(Request("g_goods_idx"),20)			'출력 갯수
...
<tr height="400"> 
	<td valign='top'>
		...
		<div class="sub_title" style="background:url(/images/common/sub_t_end.gif) top repeat-x;width:100%;"><img src="/images/common/sub01_t02.gif"></div>

		<form  id='frm_custom' name='frm_custom' method='post' action='y_custom_goods_list.asp'>
		...
		<tr height="30">			
			<td colspan='2'>
				...
				<option value="0" <% If s_g_set="0" Then %>selected <% End If %>> ::셋트유형:: </option>
				...
				<select name="s_gaetong_code"  class='txtbox' style="width:80px;">
		...
		<tr height="30"> 
			...
			<td align="right" style="border-style:none">
				...
				<img src="/images/button/btn_receipt04.gif" onclick="location.href='y_custom_goods_list.asp?s_receipt_chk=O&list_cnt=<%=list_cnt%>';" align='absmiddle'>

				<% If ad_b_y_excel="Y" Then %>
					<a href="y_custom_goods_excel.asp?<%=s_link_txt%>"><img src="/images/button/btn_excel.gif" border=0 align="absmiddle"></a>
				<% Else %>
				...
				<a href="y_custom_regist_top.asp"><img src="/images/button/btn_new.gif" align='absmiddle' border="0"></a>
				<% else %>
  ...
  strSql = " select  TOP " &top_cnt& " A.g_goods_idx, A.g_article_idx1, A.g_article_idx2, A.g_yuchi_m_id, A.g_date_yuchi, "
  strSql = strSql & " A.g_date_gaetong, A.g_writeday, A.c_tel1, A.c_tel2, A.g_sp_name, A.g_sp_price, A.g_set, A.g_article_cnt, "
  strSql = strSql & " B.m_name as yuchi_name, C.article_name , D.article_name as article_name1, A.g_consult_bigo, "
  strSql = strSql & " E.gaetong_name, A.c_name, G.promise_name, H.sale_name, A.g_sp_date_start, A.g_sp_date_end, "
  strSql = strSql & " L.dealer_name , B.m_tel1, B.m_tel2, E.gaetong_color, O.service_name, A.g_sp_give_type, "
  strSql = strSql & " K.option_name, A.g_service_no, A.g_receipt_type, A.g_consult_type, F.black_type, A.c_jumin1, A.c_jumin2, "
  strSql = strSql & " A.g_date_gaetong_hope, A.g_date_gaetong_hope_time,A.g_sp_price_top, A.g_sp_name_top  "
  strSql1 = ""
  ...
  strSql1 = strSql1 & "				where (A.g_trans_in_dealer='"&m_dealer_code&"')"
  ...
  <%
	...
	Do until objRs.EOF Or i>objRs.PageSize 
		...
		g_article_cnt		= objRs("g_article_cnt")
	...
	%>
		...
		<tr height="30" <%=bg_txt%> style="font-size:11px;" onmouseover="style.backgroundColor='#CEE3F6';this.style.cursor='hand';" onmouseout="style.backgroundColor='<%=bg_color%>'">
			...
			<td class="con4"><a href="y_custom_goods_regist.asp?g_goods_idx=<%=g_goods_idx%>&page=<%=page%>&nowblock=<%=nowblock%>&s_link_txt=<%=Server.URLEncode(s_link_txt)%>" title="<%=c_name%>, Tel.<%=c_tel2%>"><span style="<%=c_name_color%>"><b><%=Left(c_name,8)%></b></span></a></td>        
			<td class="con4"><span title="최초등록일:<%=g_writeday%>"><%=g_date_yuchi%></span></td>          
			<td class="con4"><span title="<%=dealer_name%>_<%=m_tel2%>"><%=Left(yuchi_name,5)%></span></td>          
			<td class="con2"><span title="<%=article_name%>"><%=Left(article_name,6)%></td>        
  ...
  %>
		...
		<div class="pagelist">
			...
			mychoice=10
			link_url = "y_custom_goods_list.asp?"&s_link_txt&"&"
			%>
