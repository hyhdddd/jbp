  ...
  if g_goods_idx<>"" Then  
	...
	strSql = strSql & " A.g_zipcode1, A.g_zipcode2, A.g_address, A.g_date_yuchi, A.g_service_no, A.g_code_combine, "
	strSql = strsql & " A.g_bank_cd , A.g_bank_no, A.g_bank_name, E.black_type, A.c_auth_chk, A.g_writeday, "
	...
	strSql = strSql & " A.g_code_course_idx, A.g_sp_date_start, A.g_sp_name, A.g_sp_price, "
	strSql = strSql & " F.gaetong_name, A.g_goods_ref,  A.g_card_jumin2, A.g_give_chk, "
	...
	strSql = strSql & " A.g_sum_money_chk, A.g_sum_money_txt, A.g_code_area, A.g_code_document, A.c_zipcode2, "
	strSql = strSql & " A.c_type, A.c_jumin1, A.c_jumin2, A.c_tel2, A.c_tel2_com, A.c_office_no, A.c_office_name, A.c_zipcode1,"
	...
	strSql = strSql & "			where (A.g_trans_in_dealer='"&m_dealer_code&"') and A.g_goods_idx="&g_goods_idx 	
...
<!--
	function custom_consult_reply(form_name)
	{
		...
		frm.action="y_custom_goods_consult_ok.asp";
		frm.submit();
	}
-->
</script>
...
<tr height='400' valign='top'> 
	<td>
		...
		<form id="frm_custom_regist" name="frm_custom_regist" action="y_custom_goods_regist_ok.asp" ENCTYPE="MULTIPART/FORM-DATA" METHOD="POST"> 
		...
		<tr>
			...
			<td colspan="3">
				...
				<input type="text" id="c_address" name="c_address" class="txtbox" value="<%=c_address%>"  size="80" maxlength="80" OnKeyDown="enter_chk('c_tel1');"><img src="/images/button/btn_search01.gif" Border="0" align="absmiddle" style="cursor:pointer;" onclick="sample4_execDaumPostcode('c_zipcode1','c_address');"></td>
		</tr>	
		<tr>
			<th> 전화 </th>
			<Td> 
		...
		<tr>
			...
			<td>				
				<select name="g_yuchi_m_id" style="width:150px;">
				<option value="0">::: 유치자 :::</option>
				<% 
				strSql= " select m_id, m_name,m_status from emp_list where m_dealer_code='"&m_dealer_code&"' order by m_name ASC"
				set objRs=objCnn.Execute(strSql)
		...
		<Tr>
			<th>상품선택</th>
			<td>
				<select name="g_article_idx1" style="width:100px;"
				...
				End if
				%>	
				</select><select name="g_article_idx2" style="width:120px;" onchange="option_change(this,'frm_custom_regist',this.selectedIndex,'R'); sale_change(this,'frm_custom_regist',this.selectedIndex); service_change1(this,'frm_custom_regist',this.selectedIndex); promise_change1(this,'frm_custom_regist',this.selectedIndex);">
				...
				End if
				%>	
				</select><select name="g_option_idx" style="width:130px;">
  ...
  Do Until objRs.eof
		...
		l_g_sp_give_type_txt	= fnc_g_sp_give_type(l_g_sp_give_type,"Y")
  ...
  %>
				<tr height="30" style="font-size:11px;">
					<td class="con5"><b>
						<a href="y_custom_goods_regist.asp?g_goods_idx=<%=l_g_goods_idx%>&page=<%=page%>&nowblock=<%=nowblock%>&s_link_txt=<%=Server.URLEncode(s_link_txt)%>"><span title="<%=c_name%>"><%=Left(c_name,10)%></span></a></b></td> 
  ...
  %>
		...
		<div class="button"> 
			<a href="y_custom_goods_list.asp?g_goods_idx=<%=g_goods_idx%>&page=<%=page%>&nowblock=<%=nowblock%>&<%=s_link_txt%>"><img src="/images/button/btn_list.gif" border=0 align='absmiddle'></a>

			<% If g_goods_idx<>"" Then %>
				<% if ad_b_y_custom="E" or ad_b_y_custom="A" then %>
					<span onClick="send_custom_goods_regist('frm_custom_regist','Y');">
					<img src="/images/button/btn_save.gif"  border="0" align='absmiddle' onmouseover="this.style.cursor='hand';"></span>
				<% Else %>
				...
				<% End If %>
			<% Else %>
				<% if ad_b_y_custom="W" Or ad_b_y_custom="E" or ad_b_y_custom="A" then %>
			...
			<a href="y_custom_regist_top.asp?g_goods_idx_new=<%=g_goods_idx%>"><img src="/images/button/btn_custom_add.gif" align="absmiddle"></a>
