  ...
  c_jumin_date			= WORD_CHANGE(UploadForm("c_jumin_date"),50)							'발급일자
  ...
  g_move_company		= WORD_CHANGE(UploadForm("g_move_company"),40)
  ...
  Else
	...
	If black_type="C" Then
		Alert_back " 입력하신 고객은 블랙리스트의 불량고객으로 등록되어 있으므로 청약을 받지 않는 고객입니다."
		Response.end
	End if
  End If
  ...
  IF len(trim(filename4))>1 Then
	...
	strSql = strSql & " INSERT INTO customer_goods (g_o_code, c_name, c_type, c_jumin1,c_jumin2, "
	strSql = strSql & " c_office_no, c_office_name, c_zipcode1, c_zipcode2, c_address, c_tel1, c_tel2, c_tel2_com, c_tel3, c_email,"
	strSql = strSql & " c_jumin_date, g_yuchi_m_id, g_jupsu_m_id, g_article_idx1, g_set, c_auth_chk,  "
  ...
  <%
   ...
   '------------------ 자동정산 로직 ----------------
	...
	c_g_article_cnt			= g_article_cnt3		'회선수1
  ...
  If black_type="A" Then
	  Alert_url " 입력하신 고객은 블랙리스트 고객으로 주의 고객 입니다. 참고해 주세요!\n\n 고객 상품이 되었습니다.","y_custom_goods_list.asp"
  ElseIf black_type="B" then
	  Alert_url " 입력하신 고객은 블랙리스트 고객으로 경고 고객 입니다. 참고해 주세요!\n\n 고객 상품이 등록되었습니다.","y_custom_goods_list.asp"
  else
	  Alert_url " 고객 상품이 등록되었습니다.","y_custom_goods_list.asp"
  End If
