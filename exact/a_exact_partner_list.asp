  ...
  order_chk				= WORD_CHANGE(Request("order_chk"),20)
  ...
  If order_chk="" Or IsNull(order_chk) Then
	order_chk	="Y.dealer_name Asc"
  End if
  
  If order_chk="Y.dealer_name Asc" Then 
	order_chk1="Y.dealer_name Desc"	
  Else 
	order_chk1="Y.dealer_name Asc"	
  End If 

  s_link_txt ="s_gaetong_code="&s_gaetong_code&"&date_chk="&date_chk&"&s_date_start="&s_date_start&"&s_date_end="&s_date_end&"&s_article_idx1="&s_article_idx1&"&s_article_idx2="&s_article_idx2&"&s_option_idx="&s_option_idx
...
function toggle_img(src)
{
	...
	var charge_idx	= document.getElementsByName("dealer_code")
	...
	for( var i=0; i<charge_idx.length; i++) 
		...
		if(mail_del_chk=="O")
		{
			...
			if(ans==true)
			{
				...
				NewWindow('a_exact_partner_batch.asp','a_partner_exact','600','350','no');
				frm.target="a_partner_exact";
				frm.action="a_exact_partner_batch.asp";
				frm.submit();
			}
			else
			{
...
<tr height='400' valign='top'> 
	<td>
		...
		<form id="frm_exact" name='frm_exact' method='post' action='a_exact_partner_list.asp'>
		...
		<tr>
			<td colspan='2'>
				...
				strSql = strSql & " order by article_idx Asc"
		...
		<tr>
			<td align='right'>
				<a href="a_exact_partner_excel.asp?<%=s_link_txt%>&order_chk=<%=order_chk%>"><span class="btn_m_white01"> 엑셀저장</span></a>
		...
		<tr>
			...
			<th class="head_Gray_TLB" rowspan='2'> 협력점</th>
			...
			<th class="head_Gray_TLB" rowspan='2'>사업자</th>
  ...
  strSql = "  select K.d_id, K.t_charge_promise, K.t_charge_sp_top, K.t_charge_sp, K.t_charge_sale, "
  strSql = strSql & " K.t_charge_document, K.t_charge_buga, K.t_charge_combine, "
  strSql = strSql & " K.t_charge_gita, K.t_charge_bundle, Y.dealer_name , Y.dealer_bank_name, Y.dealer_bank_no,"
  ...
  strSql = strSql & "				order by "&order_chk
  ...
  if objRs.BOF or objRs.EOF Then
	...
	Do until objRs.EOF 
		...
		dealer_bank_name		= objRs("dealer_bank_name")		'예금주
		bank_name				= objRs("bank_name")			'은행명
		dealer_bank_no			= objRs("dealer_bank_no")		'계좌번호
		dealer_com_yn			= objRs("dealer_com_yn")		'사업자유무
		
		If dealer_com_yn="Y" Then
			tax_persent			= "10%"
			d_t_charge_tax		= Fix(t_charge_sum*0.1)			
	...
	%>
		<tr height=30 onmouseover="style.backgroundColor='#CEE3F6';this.style.cursor='hand';" onmouseout="style.backgroundColor='white'">
			...
			<td class="list_LB_center"><input type="checkbox" id="dealer_code" name="dealer_code" value="<%=dealer_code%>"></td>
			<td class="list_LB_center"><a href="a_exact_list.asp?s_gaetong_code=<%=s_gaetong_code%>&s_dealer_code=<%=dealer_code%>&date_chk=<%=date_chk%>&s_date_start=<%=s_date_start%>&s_date_end=<%=s_date_end%>&s_article_idx1=<%=s_article_idx1%>&s_article_idx2=<%=s_article_idx2%>&s_option_idx=<%=s_option_idx%>"><%=dealer_name%></a></td>
			<td class="list_LB_center"><%=bank_name%></td>
			<td class="list_LB_center"><%=dealer_bank_no%></td>
			<td class="list_LB_center"><%=dealer_bank_name%></td>
			...
			<td class="list_LB_center"><%=dealer_com_yn%></td>
			<td class="list_LB_center"><%=tax_persent%></td>
			<td class="list_LB_center"><%=d_t_charge_tax%></td>
			<td class="list_LB_center"><%=d_t_charge_sum%></td>
			<td class="list_LBR_center"><a href="javascript:NewWindow('/exact_view/a_exact_regist_top.asp?<%=s_link_txt%>&dealer_code=<%=dealer_code%>','tax_view',900,700,'yes');
  ...
  %>
		<Tr height=30>
			<Td colspan='3' class="list_LB_center">
				...
				<a href="javascript:exact_submit('frm_exact');"><img src="/images/button/apply.gif" border=0></a>
			</td>
