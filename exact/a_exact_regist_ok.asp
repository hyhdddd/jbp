  ...
  c_office_no			= WORD_CHANGE(UploadForm("c_office_no"),20)
  ...
  g_date_yuchi			= WORD_CHANGE(date_null_chk(UploadForm("g_date_yuchi")),40)				'고객 상품 접수일
  ...
  g_bank_no				= WORD_CHANGE(UploadForm("g_bank_no"),50)								'계좌번호
  g_bank_name			= WORD_CHANGE(UploadForm("g_bank_name"),50)								'예금주
  g_sp_name				= WORD_CHANGE(UploadForm("g_sp_name"),50)								'사은품 정보 - 이름 
  g_sp_price			= WORD_CHANGE(money_chk(UploadForm("g_sp_price")),30)					'사은품 정보 - 금액
  ...
  g_service_no			= WORD_CHANGE(UploadForm("g_service_no"),30)							'가입서비스번호
  g_bank_name			= WORD_CHANGE(UploadForm("g_bank_name"),30)								'예금주
 ...
 '사은품 계좌정보
  ...
  g_code_service1		= WORD_CHANGE(UploadForm("g_code_service1"),20)							'부가서비스
  ...
  g_move_memo			= WORD_CHANGE(UploadForm("g_move_memo"),200)

  '본사사은품
  g_sp_name_top			= WORD_CHANGE(UploadForm("g_sp_name_top"),50)							'본사 사은품 - 이름 
  g_sp_price_top		= WORD_CHANGE(money_chk(UploadForm("g_sp_price_top")),20)				'본사 사은품 - 금액
  ...
  IF len(trim(filename4))>1 Then
	...
	strSql = strSql & ", g_sp_bank_jumin1 = '"& g_sp_bank_jumin1 &"'"
	strSql = strSql & ", g_sp_bank_jumin2 = '"& g_sp_bank_jumin2 &"'"
  ...
  <%
  Alert_url " 고객 상품이 등록되었습니다.","a_exact_list.asp?g_goods_idx="&g_goods_idx&"&page="&page&"&nowblock="&nowblock&"&"&s_link_txt
  %>