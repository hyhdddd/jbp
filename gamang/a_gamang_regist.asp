...
<!--
	...
	function custom_consult_reply(form_name)
	{
		...
		frm.action="a_gamang_consult_ok.asp";
		frm.submit();
	}
...
<tr bgcolor="#FFFFFF"  align='center'> 
	<td valign='top' align='center'>
		...
		<form action="a_gamang_regist_ok.asp" method="post" name="frm_gamang"> 
		...
		<div class="button">				
			<% If c_m_gamang_idx<>"" Then %>
				<% if ad_g_a_gamang="E" or ad_g_a_gamang="A" then %>
			...
			<% Else %>
				...
				<% End If %>
			<% End If %>						
			<a href="a_gamang_list.asp?search_key=<%=search_key%>&search_txt=<%=search_txt%>&page=<%=page%>"><img src="/images/button/btn_list.gif" border=0 align='absmiddle'></a>
			<% If c_m_gamang_idx<>"" Then %>
			<% If session_m_step="M" Then %>
				<a href="javascript:board_del_ok('a_gamang_regist_del.asp?c_m_gamang_idx=<%=c_m_gamang_idx%>')"><img src="/images/button/btn_delete.gif"  border="0" align='absmiddle'></a> 
			<% End If %>
			<% End If %>
...
%>	
		<tr height="25">
		    ...
		    <td class='con2'><%=consult_bigo%>
				<% If session_m_step="M" Then %>
				&nbsp;&nbsp;&nbsp;&nbsp;
				<span title="이력글 삭제" onclick="reply_del_ok('a_gamang_consult_del.asp?consult_idx=<%=consult_idx%>&c_m_gamang_idx=<%=c_m_gamang_idx%>');" onmouseover="this.style.cursor='hand';">
