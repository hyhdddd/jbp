...
<tr bgcolor="#FFFFFF"  align='center'> 
	<td valign='top' align='center'>
		...
		<form name='frm_gamang' method='post' action='y_gamang_list.asp'>
		<tr height=30> 
			<td align="left"  class="table_header2" style="padding-left:5">
			...
			<td align="right" valign='middle'>		
				...
				<a href="y_gamang_regist.asp"><img src="/images/button/btn_new.gif" border="0" align='absmiddle'></a>
				<% else %>
  ...
  strSql = strSql &  "				WHERE (A.c_m_m_id in (select m_id from emp_list where m_dealer_code='"&m_dealer_code&"')) "
...
%>			  
		<tr height=28>
			<td class='con5'><%=i%></td>
			<td class='con4'>
				<a href="y_gamang_regist.asp?c_m_gamang_idx=<%=c_m_gamang_idx%>&search_key=<%=search_key%>&search_txt=<%=search_txt%>&page=<%=page%>"><%=m_name%></a></td>
			<td class='con4'><%=c_m_name%></td>
			<td class='con4'><%=c_m_htel1%>&nbsp;</td>
			<td class='con4'><%=c_m_mtel1%>&nbsp;</td>
			<td class='con2'><%=c_m_address%>&nbsp;</td>
			<td class="con2"><%=c_m_consult_type%>&nbsp; <%=c_m_consult_bigo%></td>
...
%>		  
		...
		<div class="pagelist">	
			...
			mychoice=10
			link_url = "y_gamang_list.asp?search_txt="&search_txt&"&search_key="&search_key&"&"
			%>
